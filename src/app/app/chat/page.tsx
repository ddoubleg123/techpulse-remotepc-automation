  ...
  return (
    <div style={{ flex:1, overflowY:'auto', padding:'32px', display:'flex', flexDirection:'column', alignItems:'center' }}>
      <div style={{ width:'100%', maxWidth:580 }}>
        ...
        <div style={{ padding:'20px', borderRadius:16, background:'var(--bg-card)', border:'1px solid var(--border-card)', marginBottom:16 }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            <button onClick={handleVinLookup} disabled={vin.length < 10 || lookingUp}
              ...
              <Search size={14} /> {lookingUp ? 'Looking upÃÂ¢ÃÂÃÂ¦' : 'Look Up'}
            </button>
          </div>
        ...
        {/* Upload ÃÂ¢ÃÂÃÂ accept=* so all files show in picker; binary detected in JS */}
        <div onDrop={handleDrop} onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)}
          ...
          {/* accept=* ÃÂ¢ÃÂÃÂ all files visible in picker; PDF/binary rejected in handleFile */}
          ...
          {uploadedFile ? (
            ...
            <>
              ...
              <div style={{ fontSize:13, color:'var(--text-3)' }}>Drag & drop or click to browse ÃÂ¢ÃÂÃÂ all file types accepted</div>
            </>
          )}
        </div>
        ...
        {showPasteBox && (
          <div style={{ padding:'16px', borderRadius:12, background:'rgba(245,158,11,0.06)', border:'1px solid rgba(245,158,11,0.25)', marginBottom:16 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
              ...
              <span style={{ fontSize:13, fontWeight:700, color:'#f59e0b' }}>PDF detected â paste the text instead</span>
              <button onClick={() => { setShowPasteBox(false); setPastedContent(''); }} style={{ marginLeft:'auto', background:'none', border:'none', cursor:'pointer', color:'var(--text-3)', fontSize:18, lineHeight:1 }}>Ã</button>
            </div>
      ...
      <div style={{ width:'100%', maxWidth:560 }}>
        <div style={{ padding:'12px 16px', borderRadius:12, background:'var(--bg-feed)', border:'1px solid var(--border-card)', marginBottom:24, display:'flex', alignItems:'center', gap:10 }}>
          ...
          <span style={{ fontSize:13, fontWeight:600, color:'var(--text-1)' }}>
            {vehicle.year && vehicle.make ? `${vehicle.year} ${vehicle.make} ${vehicle.model}${vehicle.engine ? ' ÃÂ¢ÃÂÃÂ ' + vehicle.engine : ''}` : vehicle.vin ? `VIN: ${vehicle.vin}` : 'Vehicle'}
          </span>
        ...
        <div style={{ marginBottom:20 }}>
          ...
          <p style={{ fontSize:14, color:'var(--text-2)', margin:0 }}>{uploadedReport && validCodes.length > 0 ? 'Codes extracted from your report ÃÂ¢ÃÂÃÂ review and edit as needed.' : 'Enter fault codes from your scanner, or describe the symptoms.'}</p>
        </div>
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
      <div style={{ padding:'10px 20px', borderBottom:'1px solid var(--border-card)', background:'var(--bg-feed)', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={onBack} style={{ padding:'6px 12px', borderRadius:8, background:'var(--bg-input)', border:'1px solid var(--border-input)', color:'var(--text-2)', fontSize:12, cursor:'pointer' }}>ÃÂ¢ÃÂÃÂ Back</button>
      ...
      <div style={{ padding:'14px 20px', borderTop:'1px solid var(--border-card)', background:'var(--bg-card)', display:'flex', gap:10, flexShrink:0 }}>
        <textarea rows={1} value={input} onChange={e => setInput(e.target.value)}
          ...
          placeholder='Ask Synth a follow-up question or provide more detailsÃÂ¢ÃÂÃÂ¦'
        ...
        <div style={{ padding:'20px 24px', borderRadius:16, background:'var(--bg-card)', border:'1px solid var(--border-card)', marginBottom:14 }}>
          ...
          <div style={{ padding:'12px 14px', borderRadius:10, background:'var(--bg-feed)', border:'1px solid var(--border-card)' }}>
            ...
            <div style={{ fontSize:14, fontWeight:600, color:'var(--text-1)' }}>
              {vehicle.year && vehicle.make ? `${vehicle.year} ${vehicle.make} ${vehicle.model}${vehicle.engine?' ÃÂ¢ÃÂÃÂ '+vehicle.engine:''}` : `VIN: ${vehicle.vin}`}
            </div>
          </div>
        </div>
        {codes.length > 0 && (
    ...
    <div style={{ flex:1, overflowY:'auto', padding:'32px', display:'flex', flexDirection:'column', alignItems:'center' }}>
      <div style={{ width:'100%', maxWidth:520 }}>
        ...
        <div style={{ marginBottom:28 }}>
          ...
          <div style={{ display:'flex', gap:10 }}>
            {[{v:true,label:'Yes ÃÂ¢ÃÂÃÂ Fixed'},{v:false,label:'Not Yet'}].map(({v,label}) => (
