    ...
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', padding:'14px 24px', borderBottom:'1px solid var(--border-card)', background:'var(--bg-card)', flexShrink:0 }}>
      {steps.map((s, i) => (
        <div key={s.id} style={{ display:'flex', alignItems:'center' }}>
          ...
          {i < steps.length - 1 && (
            <div style={{ width:36, height:2, background: i < idx ? '#10b981' : 'var(--border-card)', margin:'0 4px 16px', transition:'background 0.3s' }} />
          )}
        </div>
      ))}
    </div>
...
function VinStep({ onNext }: { onNext: (vehicle: Vehicle, uploadedReport?: string) => void }) {
  ...
  const handleFile = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      // Try to extract VIN from content
      const vinMatch = content.match(/\b[A-HJ-NPR-Z0-9]{17}\b/);
  ...
  const handleVinLookup = async () => {
    ...
    setLookingUp(true);
    // Basic VIN decode — in production hits NHTSA API
    await new Promise(r => setTimeout(r, 800));
    // For now set vin in vehicle; manual fields can override
    setVehicle(v => ({ ...v, vin }));
    setLookingUp(false);
    setShowManual(true);
  };
    ...
    <div style={{ flex:1, overflowY:'auto', padding:'32px', display:'flex', flexDirection:'column', alignItems:'center' }}>
      <div style={{ width:'100%', maxWidth:580 }}>
        <div style={{ marginBottom:28 }}>
          ...
          <p style={{ fontSize:14, color:'var(--text-2)', margin:0 }}>Enter a VIN to look up the vehicle, or upload a diagnostic report to get started.</p>
        </div>
        ...
        <div style={{ padding:'20px', borderRadius:16, background:'var(--bg-card)', border:'1px solid var(--border-card)', marginBottom:16 }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            <button
              ...
              disabled={vin.length < 10 || lookingUp}
              style={{
                padding:'11px 16px', borderRadius:10, border:'none', cursor: vin.length >= 10 ? 'pointer' : 'not-allowed',
                background: vin.length >= 10 ? 'linear-gradient(135deg,#00c3ff,#0055ff)' : 'var(--bg-input)',
                color: vin.length >= 10 ? '#fff' : 'var(--text-3)',
                fontSize:13, fontWeight:700, display:'flex', alignItems:'center', gap:6, whiteSpace:'nowrap',
              }}>
              <Search size={14} /> {lookingUp ? 'Looking up…' : 'Look Up'}
            </button>
          </div>
          {vin.length > 0 && vin.length < 17 && (
            <div style={{ fontSize:11, color:'var(--text-3)', marginTop:6 }}>{17 - vin.length} characters remaining</div>
          )}

          {/* Manual vehicle fields — shown after VIN lookup or by toggling */}
          {(showManual || vin.length === 17) && (
            <div style={{ marginTop:16, paddingTop:16, borderTop:'1px solid var(--border-card)' }}>
              <div style={{ fontSize:12, fontWeight:600, color:'var(--text-3)', marginBottom:12 }}>VEHICLE DETAILS</div>
        ...
        <div
          ...
          style={{
            padding:'28px 20px', borderRadius:16, textAlign:'center', cursor: uploadedFile ? 'default' : 'pointer',
            background: dragOver ? 'rgba(0,195,255,0.06)' : uploadedFile ? 'rgba(16,185,129,0.06)' : 'var(--bg-feed)',
            border: `2px dashed ${dragOver ? 'var(--accent)' : uploadedFile ? '#10b981' : 'var(--border-card)'}`,
            transition:'all 0.2s', marginBottom:24,
          }}>
          <input ref={fileRef} type="file" accept=".txt,.csv,.pdf,.json,.xml,.rtf" style={{ display:'none' }}
          ...
          {uploadedFile ? (
            <div>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:10, marginBottom:8 }}>
                ...
                <button onClick={e => { e.stopPropagation(); setUploadedFile(null); setUploadedContent(''); setVin(''); }}
                  style={{ marginLeft:'auto', width:28, height:28, borderRadius:7, background:'rgba(239,68,68,0.1)', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
                  <X size={14} color="#f87171" />
                </button>
              </div>
              {vin && <div style={{ fontSize:12, color:'var(--text-2)', marginTop:4 }}>VIN detected: <strong style={{ color:'var(--text-1)', fontFamily:'monospace' }}>{vin}</strong></div>}
            </div>
          ) : (
            <>
              ...
              <div style={{ fontSize:13, color:'var(--text-3)' }}>Drag & drop your scanner export here, or click to browse</div>
              <div style={{ fontSize:11, color:'var(--text-3)', marginTop:6 }}>Supports .txt, .csv, .json, .pdf, .xml</div>
            </>
          )}
        </div>
        ...
        <button
          onClick={() => {
            const v = { ...vehicle, vin };
            onNext(v, uploadedContent || undefined);
          }}
          disabled={!canProceed && !uploadedFile}
          style={{
            width:'100%', padding:'14px', borderRadius:12,
            background: (canProceed || uploadedFile) ? 'linear-gradient(135deg,#00c3ff,#0055ff)' : 'var(--bg-input)',
            color: (canProceed || uploadedFile) ? '#fff' : 'var(--text-3)',
            fontSize:15, fontWeight:700, border:'none',
            cursor: (canProceed || uploadedFile) ? 'pointer' : 'not-allowed',
...
// ─── STEP 2: DTC CODES ───────────────────────────────────────────
function CodesStep({ vehicle, uploadedReport, onNext, onBack }:
  { vehicle: Vehicle; uploadedReport?: string; onNext: (codes: DtcCode[], symptoms: string) => void; onBack: () => void }
) {
  const [codes, setCodes] = useState<DtcCode[]>([{ code: '', description: '' }]);
  const [symptoms, setSymptoms] = useState('');

  // Auto-extract codes from uploaded report
  useEffect(() => {
    if (uploadedReport) {
      const matches = [...uploadedReport.matchAll(/\b(P|B|C|U)[0-9]{4}\b/gi)];
      if (matches.length > 0) {
        const extracted = matches.map(m => ({ code: m[0].toUpperCase(), description: '' }));
        setCodes(extracted);
      }
      // Extract symptoms/notes from report
      const lines = uploadedReport.split('\n').filter(l => l.trim().length > 20 && l.trim().length < 200);
      if (lines.length > 0) setSymptoms(lines.slice(0, 3).join(' '));
    }
  }, [uploadedReport]);

  const addCode = () => setCodes(p => [...p, { code: '', description: '' }]);
  const removeCode = (i: number) => setCodes(p => p.filter((_, idx) => idx !== i));
  const updateCode = (i: number, field: keyof DtcCode, val: string) =>
    setCodes(p => p.map((c, idx) => idx === i ? { ...c, [field]: val } : c));
      ...
      <div style={{ width:'100%', maxWidth:560 }}>
        ...
        <div style={{ padding:'12px 16px', borderRadius:12, background:'var(--bg-feed)', border:'1px solid var(--border-card)', marginBottom:24, display:'flex', alignItems:'center', gap:10 }}>
          ...
          <span style={{ fontSize:13, fontWeight:600, color:'var(--text-1)' }}>
            {vehicle.year && vehicle.make ? `${vehicle.year} ${vehicle.make} ${vehicle.model}` : vehicle.vin ? `VIN: ${vehicle.vin}` : 'Vehicle'}
          </span>
          {vehicle.engine && <span style={{ fontSize:13, color:'var(--text-3)' }}>— {vehicle.engine}</span>}
          {uploadedReport && <span style={{ marginLeft:'auto', padding:'2px 8px', borderRadius:6, background:'rgba(16,185,129,0.1)', border:'1px solid rgba(16,185,129,0.2)', fontSize:11, fontWeight:700, color:'#10b981' }}>Report Loaded</span>}
        </div>

        <div style={{ marginBottom:20 }}>
          <h2 style={{ fontSize:22, fontWeight:800, color:'var(--text-1)', margin:'0 0 6px' }}>DTC Codes</h2>
          <p style={{ fontSize:14, color:'var(--text-2)', margin:0 }}>
            {uploadedReport && validCodes.length > 0 ? 'Codes extracted from your report. Review and edit as needed.' : 'Add fault codes from your scanner, or describe symptoms if no codes.'}
          </p>
        </div>
        ...
        <div style={{ marginBottom:16 }}>
          {codes.map((c, i) => (
            <div key={i} style={{ display:'flex', gap:8, marginBottom:8, alignItems:'center' }}>
              ...
              <input value={c.description} onChange={e => updateCode(i, 'description', e.target.value)}
...
// ─── STEP 3: CHAT ─────────────────────────────────────────────────
function ChatStep({ vehicle, codes, symptoms, uploadedReport, sessionId, onReport, onBack }:
  { vehicle: Vehicle; codes: DtcCode[]; symptoms: string; uploadedReport?: string; sessionId: string; onReport: (report: DiagnosticReport, messages: Message[]) => void; onBack: () => void }
) {
  const initMsg = [
    ...
    uploadedReport ? `Scanner Report:\n${uploadedReport.substring(0, 2000)}` : null,
  ].filter(Boolean).join('\n');
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      const synthMsg: Message = { id: Date.now()+'s', role:'synth', content: data.response || data.message || 'Analyzing...', ts:Date.now() };
      setMessages(prev => [...prev, synthMsg]);
    } catch {
      setMessages(prev => [...prev, { id: Date.now()+'e', role:'synth', content:'Connection error — please try again.', ts:Date.now() }]);
  ...
  const buildReport = (): DiagnosticReport => ({
    ...
    confidence: 82,
    recommendedActions: ['Review findings above', 'Verify with physical inspection', 'Clear codes after repair'],
    ...
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
      ...
      <div style={{ padding:'10px 20px', borderBottom:'1px solid var(--border-card)', background:'var(--bg-feed)', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:7, height:7, borderRadius:'50%', background:'#34d399', boxShadow:'0 0 6px rgba(52,211,153,0.8)' }} />
          <span style={{ fontSize:13, fontWeight:700, color:'var(--text-1)' }}>Synth AI</span>
          {codes.length > 0 && <span style={{ padding:'2px 8px', borderRadius:6, background:'rgba(245,158,11,0.12)', border:'1px solid rgba(245,158,11,0.3)', fontSize:11, fontWeight:700, color:'#f59e0b' }}>{codes.map(c => c.code).join(' · ')}</span>}
          {uploadedReport && <span style={{ padding:'2px 8px', borderRadius:6, background:'rgba(16,185,129,0.1)', border:'1px solid rgba(16,185,129,0.2)', fontSize:11, fontWeight:700, color:'#10b981' }}>Report loaded</span>}
        </div>
        <div style={{ display:'flex', gap:8 }}>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'20px' }}>
        ...
        {loading && (
          <div style={{ display:'flex', gap:10, marginBottom:14 }}>
            <div style={iconStyle}><Zap size={14} color="#fff" fill="#fff" /></div>
            <div style={{ padding:'14px 18px', borderRadius:'16px 16px 16px 4px', background:'var(--bg-card)', border:'1px solid var(--border-card)', display:'flex', gap:5, alignItems:'center' }}>
              {[0,1,2].map(i => <div key={i} style={{ width:7, height:7, borderRadius:'50%', background:'var(--accent)', opacity: 0.5 + i * 0.25 }} />)}
            </div>
          </div>
        )}
        <div ref={bottomRef} />
      ...
      <div style={{ width:'100%', maxWidth:700 }}>
        <div style={{ padding:'20px 24px', borderRadius:16, background:'var(--bg-card)', border:'1px solid var(--border-card)', marginBottom:14 }}>
          ...
          <div style={{ padding:'12px 14px', borderRadius:10, background:'var(--bg-feed)', border:'1px solid var(--border-card)' }}>
            ...
            {vehicle.vin && vehicle.make && <div style={{ fontSize:12, color:'var(--text-3)', marginTop:2 }}>VIN: {vehicle.vin}</div>}
          </div>
        </div>
...
export default function ChatPage() {
  ...
  const restart = () => {
    ...
    setUploadedReport(undefined); setCodes([]); setSymptoms('');
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', background:'var(--bg-page)' }}>
      <StepBar step={step} />
      {step === 'vin'      && <VinStep onNext={(v, r) => { setVehicle(v); setUploadedReport(r); setStep('codes'); }} />}
      {step === 'codes'    && <CodesStep vehicle={vehicle} uploadedReport={uploadedReport} onNext={(c, s) => { setCodes(c); setSymptoms(s); setStep('chat'); }} onBack={() => setStep('vin')} />}
      {step === 'chat'     && <ChatStep vehicle={vehicle} codes={codes} symptoms={symptoms} uploadedReport={uploadedReport} sessionId={sessionId} onReport={(r, msgs) => { setReport(r); setChatMessages(msgs); setStep('report'); }} onBack={() => setStep('codes')} />}
